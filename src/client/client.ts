  ...
  function (gltf) {
    gltf.scene.traverse(function (child) {
      if ((child as THREE.Mesh).isMesh) {
        ...
        raycaster.set(rampContainerMesh.position, new THREE.Vector3(0, 1, 0));
...
cameraFolder.add(camera.position, "x", -50.0, 50.0, 0.1);
const yCameraController = cameraFolder.add(
  camera.position,
  "y",
  -50.0,
  50.0,
...
const zCameraController = cameraFolder.add(
  camera.position,
  "z",
  -50.0,
  50.0,
...
yCameraController.setValue(2.3);
zCameraController.setValue(36);
...
function animate(nowMsec: number) {
  if (projectile.position.x > 0.685 && projectile.position.x < 0.715) {
    ...
    projectile.position.y > startY - 0.015 &&
    projectile.position.y < startY + 0.015
  ) {
  ...
  if (rampContainerLoaded && arLoaded) {
    ...
    rampContainerBody.position.set(
      targetMarkerRoot.rotation.x * targetMarkerRoot.position.x -
        mainMarkerRoot.rotation.x * mainMarkerRoot.position.x,
      targetMarkerRoot.rotation.y * targetMarkerRoot.position.y -
        mainMarkerRoot.rotation.y * mainMarkerRoot.position.y,
      targetMarkerRoot.rotation.z * targetMarkerRoot.position.z -
        mainMarkerRoot.rotation.z * mainMarkerRoot.position.z
    );
...
function render() {
  ...
  if (
    intersects.length &&
    !exibindoMensagem &&
    !Math.round(projectile.cannonBody.velocity.y * 10000)
  ) {
